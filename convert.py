...
def convert_file(src_file: str, out_dir: str, song_detail: dict = None):
    ...
    try:
        with open(
            ...
            idx = json.load(f)
            if os.path.getsize(src_file) != int(
                idx[
                    {
                        ".uc": "size",
                        ".uc!": "size",
...
class SongFilename:
    def __init__(self, filename: str):
        ...
        self.name_split = self.name.split(
            {
                ".uc": "-",
                ".uc!": "-_-_",
                ".nmsf": "_",
    ...
    @property
    def id(self):
        return int(self.name_split[0])

    @property
    def bitrate(self):
        return round(
            int(self.name_split[1])
            / {
                ".uc": 1,
                ".uc!": 1,
                ".nmsf": 1000,
...
def get_song_details(song_ids: list[int]) -> list[dict]:
    ...
    for chunk, i in [
        (song_ids[i : i + API_SONG_DETAIL_CHUNK_SIZE], i)
        for i in range(0, len(song_ids), API_SONG_DETAIL_CHUNK_SIZE)
    ]:
        print(f"Getting song details: {i}/{len(song_ids)}", end="\r")
        r = session.get(
            f"http://music.163.com/api/song/detail/?ids={quote(json.dumps(chunk))}"
        )
...
def convert_folder(src_dir: str, out_dir="output", workers=WORKERS):
    ...
    with ProcessPoolExecutor(max_workers=workers) as exe:
        for file in conv_list:
            exe.submit(
                convert_file, file, out_dir, songs.get(SongFilename(file).id, None)
            )
